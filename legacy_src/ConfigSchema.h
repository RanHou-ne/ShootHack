...
// 配置项定义表（ConfigSchema）
// ============================================================================
// 【用途】在这里统一声明所有需要保存/读取的配置项。
//         新增菜单开关时，只需在下方对应的 CFG_XXX 列表中添加一行即可，
//         保存/读取/重置全部自动生效，无需修改 Config.cpp。
//
// 【宏说明】
//   CFG_BOOL  (json键名, 结构体字段, 默认值, "中文描述")
//   CFG_FLOAT (json键名, 结构体字段, 默认值, "中文描述")
//   CFG_INT   (json键名, 结构体字段, 默认值, "中文描述")
//   CFG_COLOR (json键名, 结构体字段, r, g, b, a, "中文描述")   ← 4 个独立 float
//
//   json键名   → 保存到 JSON 文件中的 key 名称
//   结构体字段 → MenuSwitches::ToggleState 的成员名
//   默认值     → ResetDefaults() 时恢复为该值
//   中文描述   → 仅用于注释说明，不参与序列化
// ============================================================================
...
#define CFG_BOOL_ITEMS \
    ...
    CFG_BOOL("FilterDead",       FilterDead,       false, "过滤死亡角色") \
    \
    /* ===== 颜色页 ===== */ \
...
// ---- 颜色数组 (float[4]) ----
// 注意：颜色值必须用 4 个独立的 float 参数传入，不能使用 {r,g,b,a} 花括号语法，
//       因为花括号内的逗号会被预处理器误解为宏参数分隔符。
#define CFG_COLOR_ITEMS \
    CFG_COLOR("EspColor",         EspColor,         1.0f, 0.55f, 0.30f, 1.0f, "ESP主色调(RGBA)") \
...
// 将 r/g/b/a 四个独立浮点数拼成 defaultValue 数组
